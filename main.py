...
articles_data = pd.read_csv('PRO-C142-Project-Boilerplate-main/articles.csv')
...
def popular_articles():
    global output
    article_info = assign_val()
    top_20_articles.append(article_info)
...
def recommended_articles():
    ...
    article_info = assign_val()
    top_10_articles.append(article_info)
    get_recommendations.drop([0], inplace=True)
    get_recommendations = get_recommendations.reset_index(drop = True)
    return jsonify({
        "status" : "success"
    }) 
